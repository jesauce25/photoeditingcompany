...
$(document).ready(function () {
    ...
    console.log("Project images component loaded for project ID:", window.projectId);
    
    // Helper function for safe logging
    function safeLog(method, ...args) {
        if (typeof window.logging !== 'undefined' && typeof window.logging[method] === 'function') {
    ...
    safeLog('info', `Project images component loaded for project ID: ${window.projectId}`);
    ...
    function updateRedoUI(row, isRedo) {
        ...
        if (isRedo) {
            if (row.find('.redo-badge').length === 0) {
                row.find('td:eq(1) a').after('<span class="redo-badge ml-2">REDO</span>');
            }
        } else {
            ...
            success: function(response) {
                try {
                    ...
                    if (data.status === 'success') {
                        ...
                        showToast('success', redoValue === '1' ? 'Image marked for redo' : 'Redo status removed');
                        ...
                        const mainTableRow = $('#imagesTable tr[data-image-id="' + imageId + '"]');
    ...
    $(document).on('change', '.assignee-select', function() {
        ...
        const previousText = previousValue ? selectElement.find('option[value="' + previousValue + '"]').text() : '';
        ...
        if (userId === "" && previousValue) {
            ...
            if (!confirm('This image is already assigned to ' + previousText + '. Do you want to reassign it to --Select Assignee--?')) {
            ...
            if (!isUserInTeam) {
                showToast('error', 'The selected assignee does not exist in the TEAM ASSIGNMENT. Please use "Assign Selected" to create a new assignment.');
        ...
        if (previousValue && previousValue != userId && userId !== "") {
            ...
            if (!confirm('This image is already assigned to ' + previousText + '. Do you want to reassign it to ' + currentAssigneeText + '?')) {
                // User clicked Cancel, reset to previous value
                selectElement.val(previousValue);
                return;
            }
        ...
        $.ajax({
            ...
            data: {
                ...
                image_id: imageId,
                user_id: userId || null, // Convert empty string to null for unassigning
                project_id: projectId
            },
            success: function(response) {
                try {
    ...
    $(document).on('change', '.estimated-hours, .estimated-minutes', function() {
        ...
        if (row.find('.image-role-select').length) {
            role = lastRoleValues[imageId] || row.find('.image-role-select').val() || '';
    ...
    $(document).on('change', '.image-role-select, .role-select', function() {
        ...
        const hours = timeValues.hours !== undefined ? timeValues.hours : (parseInt(row.find('.estimated-hours').val()) || 0);
        const minutes = timeValues.minutes !== undefined ? timeValues.minutes : (parseInt(row.find('.estimated-minutes').val()) || 0);
    ...
    $(document).on('click', '.status-step', function() {
        ...
        const isInProjectImagesTable = $(this).closest('table').attr('id') === 'imagesTable';
        ...
        $.ajax({
            ...
            success: function(response) {
                try {
                    const data = JSON.parse(response);
                    if (data.status === 'success') {
                        showToast('success', data.message || 'Images uploaded successfully');
